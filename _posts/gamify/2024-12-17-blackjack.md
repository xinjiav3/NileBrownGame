...
<body>
    <div class="container">
        ...
        <input type="range" id="betAmount" min="1" max="1000" value="100">
    ...
    <script>
        ...
        document.getElementById("betAmount").addEventListener("input", function () {
            document.getElementById("betValue").innerText = `$${this.value}`;
        });
        document.getElementById("startGame").addEventListener("click", async function () {
            try {
                const bet = document.getElementById("betAmount").value;
                const response = await fetch(`${API_URL}/start?bet=${bet}`, {
                    method: "POST",
                });
